...
export class AdminStubs {
  handle(endpoint: string, request: HttpRequest<any>): Observable<HttpResponse<any>> {
    if (request.method === 'GET') {
      if (endpoint === '') {
        // Get All
        return of(this.createResponse(200, this.getAll()));
      } else {
        const userId = endpoint.substring(endpoint.lastIndexOf('/') + 1);
        return of(this.createResponse(200, this.getUser(+userId)));
      }
    } else if (request.method === 'POST') {
      this.save(JSON.parse(request.body));
      return of(this.createResponse(200, ''));
    }
  }
  ...
  createResponse(code: number, bodyObject: any): HttpResponse<any> {
    return new HttpResponse({ status: code, body: JSON.stringify(bodyObject)});
  }
}
